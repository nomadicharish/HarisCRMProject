param(
  ...
  [string]$SmtpPortSecretId = "crm-smtp-port",
  [string]$SmtpSecureSecretId = "crm-smtp-secure",
...
$environmentVariables = "^|^NODE_ENV=production|TRUST_PROXY=1|APP_NAME=Talent Acquisition|APP_LOGIN_URL=$AppLoginUrl|FIREBASE_ENVIRONMENT=production|FIREBASE_PROD_PROJECT_ID=$projectId|FIREBASE_PROD_STORAGE_BUCKET=$storageBucket|FIREBASE_FIRESTORE_DATABASE_ID=default|CORS_ALLOWED_ORIGINS=$CorsAllowedOrigins"
$secretVariables = "DATA_ENCRYPTION_KEY_BASE64=$DataEncryptionSecretId`:latest,SMTP_HOST=$SmtpHostSecretId`:latest,SMTP_PORT=$SmtpPortSecretId`:latest,SMTP_SECURE=$SmtpSecureSecretId`:latest,SMTP_USER=$SmtpUserSecretId`:latest,SMTP_PASS=$SmtpPassSecretId`:latest,SMTP_FROM=$SmtpFromSecretId`:latest"
