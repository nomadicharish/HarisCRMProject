...
module.exports = function runPolicyServiceUnitTest() {
  ...
  assert.equal(hasScope({ role: "JUNIOR_ACCOUNTANT" }, "agent.actions.write"), false);
  assert.equal(
    hasScope({ role: "SENIOR_ACCOUNTANT", agentScopes: ["agent.actions.write"] }, "agent.actions.write"),
    true
  );
};
